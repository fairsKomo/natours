...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    res.status(200).json({
      status: 'Success',
      message: 'Email was sent successfullly!',
    });
  } catch (err) {
    ...
    return next(
      new AppError(
        'There was an error sending thie email try again later!',
        500
      )
    );
  }
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(new AppError('This token doesn`t exist or has expired!!', 500));
  }
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (user.changedPasswordAfter(decoded.iat)) {
    return next(
      new AppError('User has changed his password, log-in again!!', 401)
    );
  }
